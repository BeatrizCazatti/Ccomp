/*Escreva um programa organizado em múltiplos arquivos que calcule de forma iterativa o produto escalar entre dois vetores. Os vetores devem ser lidos a partir de arquivos texto e terão um número indeterminado de elementos. Seu programa deve calcular o produto escalar de forma iterativa do último para primeiro elemento do vetor. Os arquivos de entrada possuem o seguinte formato: a primeira linha contém o número de elementos do vetor; cada uma das demais linhas do arquivo contém um número real referente a um dos elementos do vetor em ordem a partir do elemento com índice 0.
...
main.c:
    Programa que lê o nome dos dois arquivos (um de cada vez), lê os respectivos vetores, calcula o produto escalar e apresenta o resultado com 3 casas decimais.
...
int main() {
    ...
    pvet = le_arquivo(nomeArq1);
    if (pvet != NULL) {
        // Use pvet aqui, por exemplo, para imprimir ou processar os dados
        // Lembre-se de liberar a memória quando não precisar mais dela:
        // free(pvet);
        // pvet = NULL;
        printf("oi");
    } else {
        printf("Nao foi possivel ler o arquivo ou alocar memoria.\n");
    }
